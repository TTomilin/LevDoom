...
class BasePolicy(ABC, nn.Module):
    ...
    :class:`~LevDoomLab.policy.BasePolicy`.

    A policy class typically has the following parts:

    * :meth:`~LevDoomLab.policy.BasePolicy.__init__`: initialize the policy, including \
        coping the target network and so on;
    * :meth:`~LevDoomLab.policy.BasePolicy.forward`: compute action with given \
        observation;
    * :meth:`~LevDoomLab.policy.BasePolicy.process_fn`: pre-process data from the \
        replay buffer (this function can interact with replay buffer);
    * :meth:`~LevDoomLab.policy.BasePolicy.learn`: update policy with a given batch of \
        data.
    * :meth:`~LevDoomLab.policy.BasePolicy.post_process_fn`: update the replay buffer \
        from the learning process (e.g., prioritized replay buffer needs to update \
        the weight);
    * :meth:`~LevDoomLab.policy.BasePolicy.update`: the main interface for training, \
    ...
    Since :class:`~LevDoomLab.policy.BasePolicy` inherits ``torch.nn.Module``, you can
    use :class:`~LevDoomLab.policy.BasePolicy` almost the same as ``torch.nn.Module``,
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        :return: A :class:`~LevDoomLab.data.Batch` which MUST have the following keys:
    ...
    def map_action(self, act: Union[Batch, np.ndarray]) -> Union[Batch, np.ndarray]:
        ...
        This function is called in :meth:`~LevDoomLab.data.Collector.collect` and only
    ...
    def map_action_inverse(
        ...
        """Inverse operation to :meth:`~LevDoomLab.policy.BasePolicy.map_action`.

        This function is called in :meth:`~LevDoomLab.data.Collector.collect` for
