...
class ReplayBufferManager(ReplayBuffer):
    ...
    .. seealso::
        ...
        Please refer to :class:`~LevDoomLab.data.ReplayBuffer` for other APIs' usage.
    """
    ...
    def add(
        ...
        ptrs, ep_lens, ep_rews, ep_idxs, ep_info = [], [], [], [], []
        for batch_idx, buffer_id in enumerate(buffer_ids):
            ...
            ep_info.append(batch[batch_idx].info)
        ...
        try:
            ...
            self._set_batch_for_children()
            self._meta[ptrs] = batch
        return ptrs, np.array(ep_rews), np.array(ep_lens), np.array(ep_idxs), np.array(ep_info)
