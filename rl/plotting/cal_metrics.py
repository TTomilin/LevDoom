...
if __name__ == '__main__':
    ...
    for i in range(len(LOGS[cur_group])):
        print('{} - DQN: {:.2f} +/- {:.2f}'.format(LOGS[cur_group][i],
                                                   np.mean(np.array(dqn_scores[LOGS[cur_group][i]])[:, -10:]),
                                                   np.std(np.array(dqn_scores[LOGS[cur_group][i]])[:, -10:])))
        print('{} - Rainbow: {:.2f} +/- {:.2f}'.format(LOGS[cur_group][i],
                                                       np.mean(np.array(rainbow_scores[LOGS[cur_group][i]])[:, -10:]),
                                                       np.std(np.array(rainbow_scores[LOGS[cur_group][i]])[:, -10:])))
        print('{} - PPO: {:.2f} +/- {:.2f}'.format(LOGS[cur_group][i],
                                                   np.mean(np.array(ppo_scores[LOGS[cur_group][i]])[:, -10:]),
                                                   np.std(np.array(ppo_scores[LOGS[cur_group][i]])[:, -10:])))
        print("=============================================================")
