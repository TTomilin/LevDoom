...
class BaseTrainer(ABC):
    ...
    def gen_doc(learning_type: str) -> str:
        ...
        - info dict: result returned from :func:`~LevDoomLab.trainer.gather_info`
    ...
    def __init__(
        ...
        save_policy_fn: Optional[Callable[[BasePolicy, int], None]] = None,
        ...
        self.save_policy_fn = save_policy_fn
    ...
    def reset(self) -> None:
        ...
        if self.test_collector is not None:
            ...
            self.best_epoch = self.start_epoch
            results = list(test_result['tasks'].values())
            self.best_reward, self.best_reward_std = \
                max(result['reward'] for result in results), max(result['reward_std'] for result in results)
            ...
            epoch_stat.update(
                {
                    ...
                    "reward": self.last_rew,
                    "length": int(self.last_len),
    ...
    def test_step(self) -> Tuple[Dict[str, Any], bool]:
        ...
        rew, rew_std = test_result["reward"], test_result["reward_std"]
        ...
        if self.epoch > 0 and not self.epoch % self.save_interval:
            self.save_policy_fn(self.policy, self.env_step)
    ...
    def train_step(self) -> Tuple[Dict[str, Any], Dict[str, Any], bool]:
        ...
        self.last_rew = result["reward"] if result["n/ep"] > 0 else self.last_rew
        self.last_len = result["length"] if result["n/ep"] > 0 else self.last_len
        data = {
            "env_step": str(self.env_step),
            "reward": f"{self.last_rew:.2f}",
            "length": str(int(self.last_len)),
            "n/ep": str(int(result["n/ep"])),
            "n/st": str(int(result["n/st"])),
        }
        if result["n/ep"] > 0:
            if self.test_in_train and self.stop_fn and self.stop_fn(result["reward"]):
                assert self.test_collector is not None
                test_result = test_episode(
                    self.policy, self.test_collector, self.test_fn, self.epoch,
                    self.episode_per_test, self.logger, self.env_step
                )
                if self.stop_fn(test_result["reward"]):
                    stop_fn_flag = True
                    self.best_reward = test_result["reward"]
