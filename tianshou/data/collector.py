...
class Collector(object):
    ...
    :param policy: an instance of the :class:`~LevDoomLab.policy.BasePolicy` class.
    :param env: a ``gym.Env`` environment or an instance of the
        :class:`~LevDoomLab.env.BaseVectorEnv` class.
    :param buffer: an instance of the :class:`~LevDoomLab.data.ReplayBuffer` class.
    ...
    dict or a :class:`~LevDoomLab.data.Batch` with the modified keys and values. Examples
    ...
    def __init__(
        ...
        self._extra_statistics = extra_statistics if extra_statistics else []
    ...
    def collect(
        ...
        :param int frame_skip: Number of frames skipped during collection. Default to 4.
        ...
        step_count = 0
        episode_count = 0
        n_ready_envs = len(ready_env_ids)
        episode_rews = [[] for _ in range(n_ready_envs)]
        episode_lens = [[] for _ in range(n_ready_envs)]
        episode_start_indices = [[] for _ in range(n_ready_envs)]
        episode_infos = [[] for _ in range(n_ready_envs)]
        ...
        while True:
            ...
            # collect statistics
            step_count += n_ready_envs

            if np.any(done):
                env_ind_local = np.where(done)[0]
                env_ind_global = ready_env_ids[env_ind_local]
                episode_count += len(env_ind_local)
                for index in env_ind_local:
                    episode_lens[index].append(ep_len[index] * frame_skip)
                    episode_rews[index].append(ep_rew[index])
                    episode_start_indices[index].append(ep_idx[index])
                    episode_infos[index].append(ep_info[index])
                ...
                if n_episode:
                    surplus_env_num = n_ready_envs - (n_episode - episode_count)
        ...
        for i, env in enumerate(self.env._env_fns):
            rews = episode_rews[i]
            ...
            len_mean, len_std = np.mean(lens), np.std(lens)
            ep_statistics = {}
            ep_statistics['rews'] = rews
            ep_statistics['lens'] = lens
            ep_statistics['idxs'] = idxs
            ep_statistics['reward'] = rew_mean
            ep_statistics['length'] = len_mean
            ep_statistics['reward_std'] = rew_std
            ep_statistics['length_std'] = len_std
        ...
        if task_stats:
            statistics['reward'] = mean(stats['reward'] for stats in task_stats)
            statistics['reward_std'] = mean(stats['reward_std'] for stats in task_stats)
            statistics['length'] = mean(stats['length'] for stats in task_stats)
...
class AsyncCollector(Collector):
    ...
    The arguments are exactly the same as :class:`~LevDoomLab.data.Collector`, please
    refer to :class:`~LevDoomLab.data.Collector` for more detailed explanation.
    """
