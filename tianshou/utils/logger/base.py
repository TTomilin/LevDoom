...
class BaseLogger(ABC):
    ...
    def append_statistics(self, tasks: Dict[str, Dict[str, any]], log_data: Dict[str, float], log_type: str) -> None:
        for task, stats in tasks.items():
            append_statistic(log_data, stats, task, log_type, 'reward')
            append_statistic(log_data, stats, task, log_type, 'length')
            for field in self._extra_statistics:
                append_statistic(log_data, stats, task, log_type, field)
    ...
    def log_train_data(self, collect_result: dict, step: int) -> None:
        ...
        if collect_result["n/ep"] > 0:
            if step - self.last_log_train_step >= self.train_interval:
                ...
                self.append_statistics(collect_result['tasks'], log_data, 'train')
    ...
    def log_test_data(self, collect_result: dict, step: int) -> None:
        ...
        if step - self.last_log_test_step >= self.test_interval:
            ...
            self.append_statistics(collect_result['tasks'], log_data, 'test')
...
def append_statistic(log_data: Dict[str, float], stats: Dict[str, float], task: str, log_type: str, field: str):
    if field in stats:
        log_data[f'{log_type}_{task}/{field}'] = stats[field]
        log_data[f'{log_type}_{task}/{field}_std'] = stats[f'{field}_std']
